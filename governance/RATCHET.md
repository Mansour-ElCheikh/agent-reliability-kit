...
When you first wire governance into an existing project, you will have a non-zero count almost certainly. The `/scaffold` bootstrap prompts to establish it for you (never silently); to do it by hand, run `ratchet emit` once:

```sh
# `ratchet emit` writes .governance-baseline.json directly from the current warn-counts
node <scaffold>/engine/dist/cli.js ratchet emit --repo-root . --config governance.yaml
...
1. **Reset the baseline.** A rule was added or changed and the new count is the genuine new reality. Run `ratchet emit` again and commit. Document the reason in the commit message. Reviewers should see this commit alone — it is the ratchet declaring a new floor.
