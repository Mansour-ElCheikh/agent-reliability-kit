/**
 * Skill-anatomy validation per ADR-0036 (RepoNav, 2026-04-30).
 *
...
export function validateSkillAnatomy(spec: CanonicalSpec): AnatomyError[] {
  ...
  if (description.length > DESCRIPTION_MAX) {
    errors.push({
      ...
      detail: `description is ${description.length} chars; max ${DESCRIPTION_MAX} per ADR-0036`,
    });
  }
...
export function formatAnatomyErrors(spec: CanonicalSpec, errors: AnatomyError[]): string {
  ...
  const lines = [`Canonical skill ${spec.path} fails ADR-0036 anatomy gate:`];
