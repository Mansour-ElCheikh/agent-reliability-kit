...
Build the emitter first (`pnpm -F @reliability-kit/emit-cursor build`) — `dist/cli.js` is the build output.
...
| `--canonical-root` | Override canonical source dir (default: `<kit>/canonical/`). For tests. |
