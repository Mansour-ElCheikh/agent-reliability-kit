...
Build: `pnpm -F @reliability-kit/engine build` → `dist/cli.js`.

## CLI

```sh
reliability-engine audit   [--config <path>] [--format json|toon] [--staged] [--repo-root <path>]
reliability-engine hook    [--config <path>] [--repo-root <path>]
reliability-engine ratchet emit    [--config <path>] [--repo-root <path>]
reliability-engine ratchet update  [--config <path>] [--repo-root <path>]
reliability-engine predicates list
```

Invoke via `node engine/dist/cli.js <subcommand>` or the `reliability-engine` bin after `pnpm install`.
...
Lists registered predicates (9 built-in + any adopter extensions).

## Built-in predicates (v0.1)

| Predicate | Rule it backs | Reads |
|---|---|---|
| `roadmap_reference_in_commit_message` | R1 (roadmap-first) | HEAD commit message |
| `source_file_has_co_located_test` | R2 (tdd_test_first) | file existence (sibling `*.test.*`) |
| `test_file_in_manifest_directory` | R3 | `testing-manifest.json` declared scopes |
...
```sh
pnpm -F @reliability-kit/engine test
```
