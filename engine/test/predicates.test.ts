...
function mkInput(
  ...
  // a deterministic expiry clock (ADR-0016). Default stub is sufficient otherwise.
...
| 1 | 001 | a | given when then | banana | Pending |`;
  ...
  it('does not false-positive on a table that has no Min-Invariant column', async () => {
    const md = `| Name | Value |
|---|---|
| array | object |`;
    const findings = await tasks_min_invariant(mkInput('dev/epics/001-x/tasks.md', md));
...
describe('test_shape_assertions — pytest style (deweld, ADR-0013)', () => {
...
describe('doc_validity — frontmatter contract + expiry (ADR-0016)', () => {
