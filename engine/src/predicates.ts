...
// ───────────────────────────────────────────────────────────────────────────
// Shared test-file resolution (ADR-0013 deweld — manifest-driven, multi-framework)
// ───────────────────────────────────────────────────────────────────────────
 ...
 * co-location template. ADR-0013: the test-file recognition and the
 * source→test path template both come from testing-manifest.json, defaulting
 * to the JS/TS `.test.<ext>` sibling layout so existing projects are unchanged.
 */
 ...
 * ADR-0013: a "test file" is recognised by the manifest's `testFilePatterns`
...
/** R26 — SKILL.md size soft warn (per ADR-0036). */
...
export const doc_validity: Predicate = (input) => {
  ...
  // ADR-0016: the contract is satisfied, so compute whether the window has
...
export const tasks_min_invariant: Predicate = (input) => {
  ...
  for (let ln = 0; ln < lines.length; ln++) {
    ...
    if (!inTable) {
      const idx = cols.findIndex((c) => /min[\s-]?invariant/i.test(c));
      if (idx >= 0) {
        inTable = true;
        minCol = idx;
      }
...
// ── Assertion vocabularies (ADR-0013: style inferred from extension) ─────────
 ...
 * explicit `rule.assertion_style` override (ADR-0013) — an unrecognised language
