...
export interface GovernanceRule {
  ...
  /**
   * test_shape_assertions (ADR-0013): force an assertion vocabulary instead of
   * inferring from the file extension. Built-in styles: 'vitest-jest', 'pytest'.
   * Absent → inferred from the extension (.ts/.js → vitest-jest, .py → pytest).
   */
  assertion_style?: string;
}

/**
 * testing-manifest.json shape (the fields the engine reads). ADR-0013 added
 * `testFilePatterns` (multi-framework test-file recognition) and `coLocation`
 * (the source→test path template), both optional with framework-neutral
...
export interface TestingManifest {
  ...
  /** Source→test path template for the co-location predicate. Default: '{dir}/{name}.test.{ext}'. */
...
export interface AuditContext {
  ...
  /**
   * Wall-clock "now" for time-relative predicates (doc_validity expiry, ADR-0016).
