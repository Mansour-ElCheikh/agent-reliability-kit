# ADR-0021: Ship tool-neutral git-workflow and testing conventions; R22 already-covered, DB-safety guard as principle
...
adopter. That mechanization gap is fixed in its own engine decision, ADR-0022;
this ADR records the convention decisions and relies on ADR-0022 for the
...
3. **R22 → already-covered, no new artifact.** The testing-manifest-as-contract
   ...
   here. The predicate's recognition is made stack-neutral by ADR-0022, so
...
- Does **not** ship a fifth testing predicate. The engine change that makes the
  ...
  ADR-0022, which modifies the existing three and adds none (the registry stays
  at eleven).
...
- ADR-0022 (manifest-driven testing predicates) — the engine change that makes
