...
3. npm workspaces inside one repo → boundary without multi-repo overhead. Adopters `npm install @reliability-kit/engine` or the full scaffold as they prefer.
...
├── engine/                 # @reliability-kit/engine
├── emitters/
│   ├── _lib/               # @reliability-kit/emit-lib
│   ├── claude-code/        # @reliability-kit/emit-claude-code
│   ├── cursor/             # @reliability-kit/emit-cursor
│   └── copilot/            # @reliability-kit/emit-copilot
├── scripts/
...
- **`workspace:*` protocol** is enforced (`@reliability-kit/emit-lib: workspace:*` resolves locally always; never accidentally pulls a published version mid-development).
...
- Workspaces let adopters install just the engine (`npm install @reliability-kit/engine`) without the rest. v0.2 npm-publish doesn't require restructuring.
