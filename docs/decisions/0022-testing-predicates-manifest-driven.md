# ADR-0022: Testing predicates read test-file shape from the manifest (multi-framework recognition)
...
enforce for any stack. The git + testing triangulation (ADR-0021) ran an
...
twice and R3 once on the self-test fixture), and must not add a predicate (ADR-0021
...
- Does **not** implement R22's "every declared command resolves to a runnable
  script" completeness check (logged in ADR-0021 as a separate enhancement).
- Does **not** touch the non-test predicates, the emitters, the SDLC loop, or the
  conventions (those are ADR-0021).

## Cross-reference

- ADR-0021 (git + testing conventions) — records that no new testing predicate
