...
| Input | Path (relative to the kit or adopter repo) | Purpose |
|---|---|---|
...
| `--output-root` | yes | Directory to emit into. Two common shapes: an adopter's project repo root, or the kit's own root (for self-regenerate). |
...
Team-shared files are part of the kit's seed content (copied at `init` time), not regenerated per tool.
...
Steps to add a new target tool to the kit:
...
- How adopters install emitters (cloning the kit + running `npx scaffold init` is the v0.1 path; package distribution is a v0.2+ consideration)
