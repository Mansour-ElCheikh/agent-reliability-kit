...
status: active
name: spec
description: SDLC Define phase — capture intent for a new feature as epic.md (+ tasks.md for 1-2 components, or per-component specs/*.md for 3+) + approaches.md. Scales internally on component count; no separate skill to choose. Adversarial reviewer-agent gates the artefact before human review.
purpose: |
...
writes:
  ...
  - dev/epics/{NNN}-{short-name}/approaches.md (initial; empty entries)
...
Capture intent for a new feature: `epic.md` + `approaches.md` always, plus a task surface that scales with the feature's shape — one inline `tasks.md` for 1-2 components, an independent `specs/<NNN>-<name>.md` per component for 3+.
...
|---|---|---|---|---|---|
| 1 | 001 | {description} | given {context} when {action} then {outcome} | {the concrete fact the test asserts — a count / threshold / named value / file path, e.g. `=== 3` or `contains "workspaceRoot"`; never a shape word like array/object/defined} | Pending |
```

The 6-column header is canonical and required: `build`, `review`, the `reviewer-agent`, and the `min_invariant_per_task` rule all read the `Spec` (links a task row to its component) and `Min-Invariant` columns. A `tasks.md` table missing the `Min-Invariant` column is a governance finding (it is not a silent pass).
...
### Branch B still uses one flat `tasks.md` — how the two reconcile (battle-test A3)

Branch B does **not** replace `tasks.md`; it adds the per-component `specs/*.md` *alongside*
the single flat `tasks.md` the `sdlc` bootstrap scaffolds. Division of labour:

| Surface | Holds | Read by |
|---|---|---|
| `specs/<NNN>-<name>.md` (one per component) | per-component **intent** — acceptance criteria, edge cases, out-of-scope, constraints | humans, the reviewer-agent (per-component audit) |
| `tasks.md` (one, flat) | the **executable task rows** — the canonical 6-column table (`# \| Spec \| Task \| TDD Behavior \| Min-Invariant \| Status`) | `build`, `review`, `min_invariant_per_task` |

The **`Spec` column is the link**: each task row names the component spec it implements (e.g.
`002` → `specs/002-*.md`). So `build` still drives one flat table while each row traces back to
its component spec for the detailed criteria. Order: write the `specs/*.md` first (intent),
then the `tasks.md` rows that satisfy them with each `Spec` cell pointing at the owning spec. A
Branch-B epic that has `specs/*.md` but no `tasks.md` is **incomplete** — the build phase has no
executable table to run.
...
- Every task row carries a concrete **Min-Invariant** — the exact fact its test asserts (a count, threshold, named value, or file path), never a shape word (`array`/`object`/`defined`). This is the false-green guard, governed by `min_invariant_per_task`: the table is a finding if the Min-Invariant column is absent or a cell is empty or shape-only.
...
## Initialize approaches.md

Create `dev/epics/{NNN}-{short-name}/approaches.md`:

```markdown
# Approaches — {epic name}
...
- [ ] epic.md + approaches.md written; tasks.md (Branch A) or specs/*.md (Branch B) written per the stated component count.
