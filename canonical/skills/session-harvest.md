...
| `cross-project-pattern` | A workflow pattern reusable beyond this repo | Cross-project memory store (host-level user-global dir, absolute path); never the per-project `memory/` |
...
For `bug-pattern`, `tool-config`: append to the per-project memory file. For `cross-project-pattern`: append to the host-level cross-project store by absolute path (never the per-project `memory/`).
