...
purpose: |
  ...
  probably" guidance is the no-anatomy drift mode ADR-0036 named. This
...
reads:
  ...
  - dev/epics/{epic}/adr.md (approaches already tried + rejected — do not retry)
...
9. Advance to the next task. If an approach was tried and abandoned, append it to `adr.md` (`Tried / Chose instead / Why`) so it is never retried.

## Slice discipline

- One task = one slice = one RED→GREEN→REFACTOR = one commit. If a task needs >~100 lines before a test can run, it was too big — split it (update the task table, note it).
...
| "I'll batch five tasks then test once" | Batching loses which change broke what. One slice, one green bar, one commit. |
...
- A commit with a red bar, or >1 task per commit.
...
- [ ] Every commit left the full suite green; one task per commit.
- [ ] Refactor steps changed structure only, never behaviour or tests.
- [ ] Abandoned approaches appended to adr.md; epic.md Status → Built on completion.
