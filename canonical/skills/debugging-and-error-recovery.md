...
purpose: |
  ...
  is wired to this kit's TDD layer (the `build` skill's RED).
...
reads:
  ...
  - dev/epics/{epic}/approaches.md (was this approach already tried + abandoned?)
...
### 3. Root cause
State the cause as one sentence: "X fails because Y." If the sentence needs an "and" or a "probably", keep isolating — you have a symptom, not the cause. Check `approaches.md`: was this approach already tried and abandoned? If so, do not retry it.
...
- Retrying an approach already recorded as abandoned in approaches.md.
...
- [ ] Abandoned approaches appended to approaches.md.
