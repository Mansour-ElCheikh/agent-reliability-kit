...
The human-facing **narration layer** over `scaffold-engine audit` + the hook-diagnostic flow. The engine does the evaluation (reads `governance.yaml`, runs predicates, compares the ratchet); this skill runs it, presents the findings severity-sorted, diagnoses why the write-time hook didn't fire, and suggests next actions.
...
```bash
# the scaffold ships this engine (S6.3b):
