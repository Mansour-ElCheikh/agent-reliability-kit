...
[![Status](https://img.shields.io/badge/status-v0.1.3--dev-orange.svg)](#status)
...
ENGINE:    YAML rules  →  PreToolUse hook (write-time gate)
           ...
           13 deterministic predicates ship built-in
...
RECORD:    ADRs (decisions; the decision_keyword_without_adr predicate
           ...
           26 ADRs ship in this repo as worked examples
```

Factory produces; engine gates; ADRs record. The product is what falls out the top.

**Human-in-the-loop by design.** The gate carries what is mechanizable: correctness (TDD, Min-Invariants, a green suite), integration (a required e2e layer for user-facing surfaces), no secrets. The human carries what no deterministic gate can encode: taste and product-judgment. Autonomy is **supervised**: the agent runs the gated loop; the human owns the Define / ship gates and the qualitative call. (A fully-autonomous, **unsupervised** mode is a deliberate, gated-behind-proof bet, not a current claim.)
...
**13 deterministic predicates ship built-in:** `boundary_imports`, `prompt_size_warn`, `doc_validity`, `scope_containment`, `no_secrets`, `source_file_has_co_located_test`, `source_file_has_test`, `test_file_in_manifest_directory`, `roadmap_reference_in_commit_message`, `decision_keyword_without_adr`, `tasks_min_invariant`, `test_shape_assertions`, `userfacing_integration_layer`. Adopters extend via `engine/extensions/*.predicates.{js,mjs}`.
...
KIT_DIR="$(pwd)"                                     # remember where the kit lives

# In your project
cd /path/to/your-project
node "$KIT_DIR/scripts/dist/scaffold.js"            # auto-detect tools, run emitters
git add -A && git commit -m "chore: bootstrap agent-reliability-kit"
```

`scaffold.ts` detects which tools the repo uses, runs each applicable emitter, and prompts (never silently) before establishing the ratchet baseline. Mixed-tool teams get the right per-tool configs on clone.
...
- **Not hands-off autonomy.** The agent runs the gated loop; the human owns the Define / ship gates and the taste/product-judgment a deterministic gate cannot reward. The gate covers correctness + integration; the qualitative layer is the human's. Fully-autonomous, unsupervised mode is a gated-behind-proof bet, not a claim.

## Status

`0.1.3-dev`. Canonical layer (12 skills + 1 subagent) is the sole source of truth; three emitters (Claude Code, Cursor, Copilot) and the rules engine (13 predicates, hook, audit, ratchet) self-test in CI. **The kit governs itself:** its own rules run against its own tree in CI (`governance.yaml`, advisory + ratchet, green), not just a fixture. Governance ships a Universal Default Set enabled (advisory under `solo`, ramping to blocking under `team`; see [ADR-0007](docs/decisions/0007-universal-rules-ship-enabled.md)).
