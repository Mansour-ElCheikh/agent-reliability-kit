...
See [ONBOARDING.md](ONBOARDING.md). Quick recap: `corepack enable && pnpm install --frozen-lockfile && pnpm run build`, then `pnpm -F @reliability-kit/engine test` for the engine self-test, and `bash emitters/<name>/test/run-test.sh` for the per-emitter golden tests.
