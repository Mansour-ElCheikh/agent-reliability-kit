// Tests for the ADR-0025 numeric-count reconciliation gate.
...
test('reconcileCounts flags a claim that disagrees with runtime reality', () => {
  const groundTruth = { predicates: 12, adrs: 25, skills: 12, subagents: 1 };
  const claims = [
    { category: 'adrs', value: 16, surface: 'README.md', line: 23, text: '16 ADRs ship' },
    { category: 'predicates', value: 12, surface: 'README.md', line: 19, text: '12 predicates' },
  ];
  const { mismatches, checked } = reconcileCounts({ groundTruth, claims });
  assert.equal(checked, 2);
  assert.equal(mismatches.length, 1);
  assert.equal(mismatches[0].category, 'adrs');
  assert.equal(mismatches[0].value, 16);
  assert.equal(mismatches[0].expected, 25);
});

test('reconcileCounts passes when every claim matches runtime reality', () => {
  const groundTruth = { predicates: 12, adrs: 25, skills: 12, subagents: 1 };
  const claims = [
    { category: 'adrs', value: 25, surface: 'README.md', line: 23, text: '25 ADRs' },
